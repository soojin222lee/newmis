...
// AI GUIDE: 선투입 프로젝트 정보 화면입니다.
// - 사내 프로젝트와 동일한 화면 구조를 기반으로 선투입 프로젝트 라벨과 데이터를 보여줍니다.
// - 목록, 등록, 상세, 결재 상신/승인 처리 흐름을 목업으로 확인합니다.
// - AI 화면 가이드는 선투입 프로젝트 등록 기준, 예산 항목, 승인 상태를 중심으로 설명해야 합니다.

let advView = 'list';
let advSelectedId = 'ADV-001';
let advBudgetRows = [{ name:'', materials:0, labor:0, outsource:0, expense:0 }];
let advSearchQuery = '';
let advStatusFilter = '';
...
// ── 진입점 ──
function renderAdvanceProject() {
  if (advView === 'list')          renderAdvList();
  else if (advView === 'register') renderAdvRegister();
  else renderAdvDetail();
}
...
function renderAdvList() {
  ...
  const filtered = Object.entries(ADVANCE_PROJECTS).filter(([, p]) => {
    const matchQ = !q || [p.name, p.code, p.projType, p.requestDept, p.responseDept, p.overview]
      .some(v => v && v.toLowerCase().includes(q));
    const matchS = !advStatusFilter || p.status === advStatusFilter;
    return matchQ && matchS;
  });

  const allStatuses = [...new Set(Object.values(ADVANCE_PROJECTS).map(p => p.status))];
  const statusOpts = ['', ...allStatuses].map(s =>
    `<option value="${s}" ${s===advStatusFilter?'selected':''}>${s||'전체 상태'}</option>`
  ).join('');

  const rows = filtered.length ? filtered.map(([id, p]) => {
    const st    = ADV_STATUS_STYLE[p.status] || ADV_STATUS_STYLE['기안중'];
  ...
  el.innerHTML = `
    <div class="page-header" style="display:flex;align-items:flex-start;justify-content:space-between">
      <div>
        <div class="page-title">선투입 프로젝트</div>
        <div class="page-sub">선투입 프로젝트 등록·조회</div>
      </div>
      <button class="save-btn" onclick="advBudgetRows=[{name:'',materials:0,labor:0,outsource:0,expense:0}];advView='register';renderAdvRegister()" style="margin-top:4px;font-size:12px;padding:6px 14px">+ 프로젝트 등록</button>
    </div>
    ...
    <div class="proj-list-toolbar">
      <div class="proj-search-wrap">
        ...
        <input class="proj-search-input" placeholder="프로젝트명, 유형, 요청부서, 개요 검색…"
          ...
          oninput="advSearchQuery=this.value;renderAdvList()">
      </div>
      <select class="proj-filter-select" onchange="advStatusFilter=this.value;renderAdvList()">${statusOpts}</select>
...
function renderAdvDetail() {
  const el = document.getElementById('s-advance-project');
  const p = ADVANCE_PROJECTS[advSelectedId];
  const st = ADV_STATUS_STYLE[p.status] || ADV_STATUS_STYLE['기안중'];

  el.innerHTML = `
    <div class="wg-detail-topbar">
      <button class="mc-back-btn" onclick="closeAdvDetail()">← 목록</button>
      <div style="flex:1">
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      <div style="padding:16px 20px">${buildAdvApproval(p.approval)}</div>
    </div>
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      <div style="padding:0 20px 20px;overflow-x:auto">${buildAdvBudgetTable(p.budget)}</div>
    </div>
    ...
    <div class="reg-erp-bar">
      ...
      <button class="save-btn" onclick="approveAdvProject('${advSelectedId}')">✅ 승인 처리</button>
    </div>` : p.status === '승인' ? `
...
function buildAdvApproval(approval) {
  const statusStyle = {
    ...
    <table class="wg-cost-table" style="min-width:600px;margin-top:12px">
      ...
      <tbody>
        ...
        <tr class="wg-tbl-total">
          ...
          <td class="wg-tbl-num adv-tbl-cur">${fmt(total)}</td>
...
//  선투입 프로젝트 등록 폼
// ════════════════════════════════════════
function renderAdvRegister() {
  const el = document.getElementById('s-advance-project');
  el.innerHTML = `
    <div class="wg-detail-topbar">
      <button class="mc-back-btn" onclick="advView='list';renderAdvList()">← 목록</button>
      <div style="flex:1">
        <div style="font-size:18px;font-weight:800;color:#1e293b">선투입 프로젝트 등록</div>
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      <div class="reg-fields-grid" style="padding:20px">
        <div class="reg-field-group">
          ...
          <input class="reg-input" id="adv-reg-name" placeholder="프로젝트명 입력">
        </div>
        <div class="reg-field-group">
          <label class="reg-label">프로젝트 유형 <span class="reg-required">*</span></label>
          <select class="reg-select" id="adv-reg-type">
            <option value="">선택</option>
            <option>선투입·전사IT운영</option>
            <option>선투입·IT지원</option>
            <option>선투입·인프라</option>
            <option>선투입·보안</option>
            <option>선투입·기타</option>
          </select>
        </div>
        <div class="reg-field-group">
          <label class="reg-label">요청부서 <span class="reg-required">*</span></label>
          <input class="reg-input" id="adv-reg-req" placeholder="요청부서명">
        </div>
        <div class="reg-field-group">
          <label class="reg-label">책임부서</label>
          <input class="reg-input" id="adv-reg-res" placeholder="미입력 시 요청부서와 동일">
        </div>
        <div class="reg-field-group" style="grid-column:1/-1">
          <label class="reg-label">프로젝트 기간 <span class="reg-required">*</span></label>
          <div style="display:flex;gap:8px;align-items:center">
            <input type="date" class="reg-input" id="adv-reg-start" style="flex:1">
            <span style="color:#94a3b8">~</span>
            <input type="date" class="reg-input" id="adv-reg-end" style="flex:1">
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="card" style="margin-bottom:16px">
      ...
      <div style="padding:16px 20px">
        <textarea class="reg-textarea" id="adv-reg-overview" rows="3" placeholder="프로젝트 목적 및 내용을 간략히 입력하세요."></textarea>
      </div>
    </div>
    ...
    <div class="card" style="margin-bottom:24px">
      ...
      <div style="padding:16px 20px;overflow-x:auto">
        <table class="wg-cost-table" style="min-width:620px" id="adv-budget-input-table">
          <thead>
            <tr>
          ...
          <tbody id="adv-budget-tbody"></tbody>
        </table>
        <button class="reg-budget-add-btn" onclick="addAdvBudgetRow()">＋ 항목 추가</button>
      </div>
    </div>

    <div class="reg-footer-bar">
      <button class="reset-btn" onclick="advView='list';renderAdvList()">취소</button>
      <button class="save-btn" onclick="submitAdvDraft()">📨 결재 상신</button>
    </div>`;

  renderAdvBudgetInputRows();
}

function renderAdvBudgetInputRows() {
  const tbody = document.getElementById('adv-budget-tbody');
  if (!tbody) return;
  tbody.innerHTML = advBudgetRows.map((r, i) => `
    <tr>
      <td style="padding:4px 8px">
        <input class="reg-input reg-input-sm" id="advb-name-${i}" value="${r.name}" placeholder="항목명" style="min-width:140px">
      </td>
      ${['materials','labor','outsource','expense'].map(k => `
      <td style="padding:4px 8px">
        <input class="reg-input reg-input-sm" id="advb-${k}-${i}" type="number" min="0" value="${r[k]||''}"
          placeholder="0" oninput="updateAdvSubtotal(${i})" style="width:100px;text-align:right">
      </td>`).join('')}
      <td class="wg-tbl-num wg-tbl-cur" id="advb-sub-${i}" style="min-width:90px">
        ${r.materials||r.labor||r.outsource||r.expense ? fmt(r.materials+r.labor+r.outsource+r.expense) : '—'}
      </td>
      <td style="padding:4px 6px;text-align:center">
        ${advBudgetRows.length > 1 ? `<button class="reg-del-btn" onclick="removeAdvBudgetRow(${i})">✕</button>` : ''}
      </td>
    </tr>`).join('');
}

function updateAdvSubtotal(i) {
  const keys = ['materials','labor','outsource','expense'];
  const sum = keys.reduce((s, k) => s + (Number(document.getElementById(`advb-${k}-${i}`)?.value) || 0), 0);
  const cell = document.getElementById(`advb-sub-${i}`);
  if (cell) cell.textContent = sum ? fmt(sum) : '—';
  advBudgetRows[i] = {
    name: document.getElementById(`advb-name-${i}`)?.value || '',
    ...Object.fromEntries(keys.map(k => [k, Number(document.getElementById(`advb-${k}-${i}`)?.value) || 0])),
  };
}

function addAdvBudgetRow() {
  // 현재 입력값 저장
  advBudgetRows.forEach((_, i) => {
    advBudgetRows[i] = {
      name: document.getElementById(`advb-name-${i}`)?.value || '',
      materials: Number(document.getElementById(`advb-materials-${i}`)?.value) || 0,
      labor:     Number(document.getElementById(`advb-labor-${i}`)?.value)     || 0,
      outsource: Number(document.getElementById(`advb-outsource-${i}`)?.value) || 0,
      expense:   Number(document.getElementById(`advb-expense-${i}`)?.value)   || 0,
    };
  });
  advBudgetRows.push({ name:'', materials:0, labor:0, outsource:0, expense:0 });
  renderAdvBudgetInputRows();
}

function removeAdvBudgetRow(idx) {
  if (advBudgetRows.length <= 1) return;
  advBudgetRows.splice(idx, 1);
  renderAdvBudgetInputRows();
}

function submitAdvDraft() {
  const name  = document.getElementById('adv-reg-name').value.trim();
  const type  = document.getElementById('adv-reg-type').value;
  const req   = document.getElementById('adv-reg-req').value.trim();
  const start = document.getElementById('adv-reg-start').value;
  const end   = document.getElementById('adv-reg-end').value;
  if (!name || !type || !req || !start || !end) {
    showToast('필수 항목(*)을 모두 입력해주세요.'); return;
  }

  // 예산 행 수집
  const budget = advBudgetRows.map((_, i) => {
    const rowName = document.getElementById(`advb-name-${i}`)?.value.trim() || `항목${i+1}`;
    const mat = Number(document.getElementById(`advb-materials-${i}`)?.value) || 0;
    const lab = Number(document.getElementById(`advb-labor-${i}`)?.value)     || 0;
    const out = Number(document.getElementById(`advb-outsource-${i}`)?.value) || 0;
    const exp = Number(document.getElementById(`advb-expense-${i}`)?.value)   || 0;
  ...
  const res     = document.getElementById('adv-reg-res').value.trim()      || req;
  const overview= document.getElementById('adv-reg-overview').value.trim() || '';
  const idx     = Object.keys(ADVANCE_PROJECTS).length + 1;
  const newId   = `ADV-${String(idx).padStart(3,'0')}`;
  const code    = `IV${108000 + idx}`;
  const today   = new Date().toISOString().slice(0,10);
  const now     = today + ' ' + new Date().toTimeString().slice(0,5);

  ADVANCE_PROJECTS[newId] = {
...
function approveAdvProject(id) {
  const p = ADVANCE_PROJECTS[id];
  if (!p) return;
